...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\loren\\OneDrive\\Documenti\\Oxford - MSc Neuroscience\\Trinity Term\\Research Project\\Code - Task, Input, Analysis\\rotation_task_v2\\rotation_task_training_lastrun.py',
...
text_move_s2 = visual.TextStim(win=win, name='text_move_s2',
    text='To move the shield around, you can use the "k" and "l" keys on your keyboard. Try moving the shield now!',
    font='Open Sans',
...
instructed_moveClock = core.Clock()
...
shield_bg_short_move_i = visual.ShapeStim(
    win=win, name='shield_bg_short_move_i', vertices=shieldCoords,units='cm', 
    size=(1, 1),
    ori=1.0, pos=(0, -0.5), anchor='center',
    lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
    opacity=None, depth=-3.0, interpolate=True)
...
radio_size = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-4.0)
text_size_s2 = visual.TextStim(win=win, name='text_size_s2',
    text='To change the shield size, you can use the "a" and "s" keys on your keyboard. Try changing the shield size now!\n\n\n\n\n\n\n\nIf you have understood how to change the shield size,\xa0\npress the space bar to advance to the next screen.',
    font='Open Sans',
...
varianceInstructionsClock = core.Clock()
...
for thisComponent in instructions_2Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('text_instructions_2.started', text_instructions_2.tStartRefresh)
thisExp.addData('text_instructions_2.stopped', text_instructions_2.tStopRefresh)
thisExp.addData('shield_miss_earth.started', shield_miss_earth.tStartRefresh)
thisExp.addData('shield_miss_earth.stopped', shield_miss_earth.tStopRefresh)
# check responses
...
thisExp.addData('key_resp_i2.started', key_resp_i2.tStartRefresh)
thisExp.addData('key_resp_i2.stopped', key_resp_i2.tStopRefresh)
thisExp.nextEntry()
...
# keep track of which components have finished
practiceMoveComponents = [shield_move, shield_centre_move, shield_bg_short_move, radioactive_move, text_move_s2, text_advance_move, key_resp_move]
...
while continueRoutine:
    ...
    # *text_move_s2* updates
    if text_move_s2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        text_move_s2.frameNStart = frameN  # exact frame index
        text_move_s2.tStart = t  # local t and not account for scr refresh
        text_move_s2.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(text_move_s2, 'tStartRefresh')  # time at next scr refresh
        text_move_s2.setAutoDraw(True)
...
thisExp.addData('text_move_s2.started', text_move_s2.tStartRefresh)
thisExp.addData('text_move_s2.stopped', text_move_s2.tStopRefresh)
...
thisExp.addData('key_resp_move.started', key_resp_move.tStartRefresh)
thisExp.addData('key_resp_move.stopped', key_resp_move.tStopRefresh)
thisExp.nextEntry()
...
for thisMove_training in move_training:
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    instructed_moveClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine:
        ...
        tThisFlip = win.getFutureFlipTime(clock=instructed_moveClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        #first, find out if L/R keys have been *released*
    ...
    for thisComponent in instructed_moveComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    move_training.addData('shield_move_i.started', shield_move_i.tStartRefresh)
    move_training.addData('shield_move_i.stopped', shield_move_i.tStopRefresh)
    move_training.addData('shield_centre_move_i.started', shield_centre_move_i.tStartRefresh)
    move_training.addData('shield_centre_move_i.stopped', shield_centre_move_i.tStopRefresh)
    move_training.addData('shield_bg_short_move_i.started', shield_bg_short_move_i.tStartRefresh)
    move_training.addData('shield_bg_short_move_i.stopped', shield_bg_short_move_i.tStopRefresh)
    move_training.addData('target_move_i.started', target_move_i.tStartRefresh)
    move_training.addData('target_move_i.stopped', target_move_i.tStopRefresh)
    move_training.addData('radioactive_move_i.started', radioactive_move_i.tStartRefresh)
    move_training.addData('radioactive_move_i.stopped', radioactive_move_i.tStopRefresh)
    move_training.addData('text_move_i.started', text_move_i.tStartRefresh)
    move_training.addData('text_move_i.stopped', text_move_i.tStopRefresh)
...
# keep track of which components have finished
practiceSizeComponents = [shield_size, shield_centre_size, shield_bg_short_size, radio_size, text_size_s2, key_resp_size]
...
while continueRoutine:
    ...
    # *text_size_s2* updates
    if text_size_s2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        text_size_s2.frameNStart = frameN  # exact frame index
        text_size_s2.tStart = t  # local t and not account for scr refresh
        text_size_s2.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(text_size_s2, 'tStartRefresh')  # time at next scr refresh
        text_size_s2.setAutoDraw(True)
...
thisExp.addData('text_size_s2.started', text_size_s2.tStartRefresh)
thisExp.addData('text_size_s2.stopped', text_size_s2.tStopRefresh)
# check responses
...
thisExp.addData('key_resp_size.started', key_resp_size.tStartRefresh)
thisExp.addData('key_resp_size.stopped', key_resp_size.tStopRefresh)
thisExp.nextEntry()
...
for thisComponent in rewardComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('reward_bar_image.started', reward_bar_image.tStartRefresh)
thisExp.addData('reward_bar_image.stopped', reward_bar_image.tStopRefresh)
...
thisExp.addData('text_reward_2.started', text_reward_2.tStartRefresh)
thisExp.addData('text_reward_2.stopped', text_reward_2.tStopRefresh)
# check responses
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
easyBlocksClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "easyBlocks"-------
while continueRoutine:
    # get current time
    t = easyBlocksClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=easyBlocksClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
for thisComponent in easyBlocksComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('text_easyBlocks.started', text_easyBlocks.tStartRefresh)
thisExp.addData('text_easyBlocks.stopped', text_easyBlocks.tStopRefresh)
# check responses
...
thisExp.addData('key_resp_easyBlocks.started', key_resp_easyBlocks.tStartRefresh)
thisExp.addData('key_resp_easyBlocks.stopped', key_resp_easyBlocks.tStopRefresh)
thisExp.nextEntry()
...
blocks = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimgen/blocks_training1_part1.csv'),
...
for thisBlock in blocks:
    ...
    while continueRoutine:
        ...
        if progress_bar_edge.status == STARTED:
            if frameN >= (progress_bar_edge.frameNStart + nFrames):
                ...
                win.timeOnFlip(progress_bar_edge, 'tStopRefresh')  # time at next scr refresh
    ...
    blocks.addData('reward_bar_red.started', reward_bar_red.tStartRefresh)
    blocks.addData('reward_bar_red.stopped', reward_bar_red.tStopRefresh)
    blocks.addData('reward_bar.started', reward_bar.tStartRefresh)
    blocks.addData('reward_bar.stopped', reward_bar.tStopRefresh)
    blocks.addData('progress_bar_edge.started', progress_bar_edge.tStartRefresh)
    blocks.addData('progress_bar_edge.stopped', progress_bar_edge.tStopRefresh)
    ...
    blocks.addData('reward_text_top.started', reward_text_top.tStartRefresh)
    blocks.addData('reward_text_top.stopped', reward_text_top.tStopRefresh)
    blocks.addData('reward_text_bottom.started', reward_text_bottom.tStartRefresh)
    blocks.addData('reward_text_bottom.stopped', reward_text_bottom.tStopRefresh)
    blocks.addData('start_text.started', start_text.tStartRefresh)
    blocks.addData('start_text.stopped', start_text.tStopRefresh)
    blocks.addData('end_text.started', end_text.tStartRefresh)
    blocks.addData('end_text.stopped', end_text.tStopRefresh)
    ...
    blocks.addData('reward_text.started', reward_text.tStartRefresh)
    blocks.addData('reward_text.stopped', reward_text.tStopRefresh)
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
varianceInstructionsClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "varianceInstructions"-------
while continueRoutine:
    # get current time
    t = varianceInstructionsClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=varianceInstructionsClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
for thisComponent in varianceInstructionsComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('text_variance.started', text_variance.tStartRefresh)
thisExp.addData('text_variance.stopped', text_variance.tStopRefresh)
# check responses
...
thisExp.addData('key_resp_variance.started', key_resp_variance.tStartRefresh)
thisExp.addData('key_resp_variance.stopped', key_resp_variance.tStopRefresh)
thisExp.nextEntry()
...
blocks2 = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('stimgen/blocks_training1_part2.csv'),
...
for thisBlocks2 in blocks2:
    ...
    for thisComponent in blockStartTextComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    blocks2.addData('radioactive_block_source.started', radioactive_block_source.tStartRefresh)
    blocks2.addData('radioactive_block_source.stopped', radioactive_block_source.tStopRefresh)
    blocks2.addData('text_2.started', text_2.tStartRefresh)
    blocks2.addData('text_2.stopped', text_2.tStopRefresh)
    # check responses
    if key_resp_blockStart.keys in ['', [], None]:  # No response was made
        key_resp_blockStart.keys = None
    ...
    blocks2.addData('key_resp_blockStart.started', key_resp_blockStart.tStartRefresh)
    blocks2.addData('key_resp_blockStart.stopped', key_resp_blockStart.tStopRefresh)
    # the Routine "blockStartText" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    triggerValue = 0;
    sendTrigger = False;
    blocks2.addData('earth_background.started', earth_background.tStartRefresh)
    blocks2.addData('earth_background.stopped', earth_background.tStopRefresh)
    blocks2.addData('harmless_area.started', harmless_area.tStartRefresh)
    blocks2.addData('harmless_area.stopped', harmless_area.tStopRefresh)
    blocks2.addData('shield.started', shield.tStartRefresh)
    blocks2.addData('shield.stopped', shield.tStopRefresh)
    blocks2.addData('shield_centre.started', shield_centre.tStartRefresh)
    blocks2.addData('shield_centre.stopped', shield_centre.tStopRefresh)
    blocks2.addData('shield_bg_short.started', shield_bg_short.tStartRefresh)
    blocks2.addData('shield_bg_short.stopped', shield_bg_short.tStopRefresh)
    blocks2.addData('laser.started', laser.tStartRefresh)
    blocks2.addData('laser.stopped', laser.tStopRefresh)
    blocks2.addData('laser_long.started', laser_long.tStartRefresh)
    blocks2.addData('laser_long.stopped', laser_long.tStopRefresh)
    blocks2.addData('radioactive.started', radioactive.tStartRefresh)
    blocks2.addData('radioactive.stopped', radioactive.tStopRefresh)
    blocks2.addData('reward_bar_red.started', reward_bar_red.tStartRefresh)
    blocks2.addData('reward_bar_red.stopped', reward_bar_red.tStopRefresh)
    blocks2.addData('reward_bar.started', reward_bar.tStartRefresh)
    blocks2.addData('reward_bar.stopped', reward_bar.tStopRefresh)
    blocks2.addData('progress_bar_edge.started', progress_bar_edge.tStartRefresh)
    blocks2.addData('progress_bar_edge.stopped', progress_bar_edge.tStopRefresh)
    blocks2.addData('progress_bar.started', progress_bar.tStartRefresh)
    blocks2.addData('progress_bar.stopped', progress_bar.tStopRefresh)
    blocks2.addData('reward_text_top.started', reward_text_top.tStartRefresh)
    blocks2.addData('reward_text_top.stopped', reward_text_top.tStopRefresh)
    blocks2.addData('reward_text_bottom.started', reward_text_bottom.tStartRefresh)
    blocks2.addData('reward_text_bottom.stopped', reward_text_bottom.tStopRefresh)
    blocks2.addData('start_text.started', start_text.tStartRefresh)
    blocks2.addData('start_text.stopped', start_text.tStopRefresh)
    blocks2.addData('end_text.started', end_text.tStartRefresh)
    blocks2.addData('end_text.stopped', end_text.tStopRefresh)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisComponent in blockEndTextComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    blocks2.addData('textPause.started', textPause.tStartRefresh)
    blocks2.addData('textPause.stopped', textPause.tStopRefresh)
    blocks2.addData('reward_text.started', reward_text.tStartRefresh)
    blocks2.addData('reward_text.stopped', reward_text.tStopRefresh)
    blocks2.addData('textContinue.started', textContinue.tStartRefresh)
    blocks2.addData('textContinue.stopped', textContinue.tStopRefresh)
    # check responses
    if key_resp_blockEnd.keys in ['', [], None]:  # No response was made
        key_resp_blockEnd.keys = None
    ...
    blocks2.addData('key_resp_blockEnd.started', key_resp_blockEnd.tStartRefresh)
    blocks2.addData('key_resp_blockEnd.stopped', key_resp_blockEnd.tStopRefresh)
    # the Routine "blockEndText" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
